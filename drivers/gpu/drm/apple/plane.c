...
static struct dcp_image_layout apple_get_interchange_layout(u32 w, u32 h,
								  u32 bpp, u32 p,
								  bool yuv)
{
	u32 meta_tile_w, meta_tile_h;
	struct dcp_image_layout layout;

	/*
	 * Compression subtiles are always 16x16 with 8 bytes of metadata,
	 * but IOMFB requires its structs to be populated with values
	 * relative to the image tile dimensions.
	 */
	if (yuv && p == 0) {
		/* Luma image tiles are 32x32... */
		layout.tile_w = layout.tile_h = 32;
		/* ...and therefore have 4x 8-byte compression subtiles */
		layout.meta_tile_bytes = 32;
	} else {
		layout.tile_w = layout.tile_h = 16;
	...
	meta_tile_w = DIV_ROUND_UP(roundup_pow_of_two(w), layout.tile_w);
	meta_tile_h = DIV_ROUND_UP(roundup_pow_of_two(h), layout.tile_h);

	layout.meta_size = ALIGN(meta_tile_w * meta_tile_h * layout.meta_tile_bytes,
				  APPLE_GPU_CACHELINE);
				    ...
				    struct drm_atomic_state *state)
{
	...
	case DRM_FORMAT_MOD_APPLE_INTERCHANGE:
		...
		for (u32 i = 0; i < fb->format->num_planes; i++) {
			...
			u32 req_size;
			struct dcp_image_layout l = apple_get_interchange_layout(w, h, bpp, i,
										 fb->format->is_yuv);
			struct drm_gem_dma_object *obj = drm_fb_dma_get_gem_obj(fb, i);

			req_size = fb->offsets[0] + l.meta_offset + l.meta_size;
				      ...
				      struct drm_atomic_state *state)
{
	...
	for (int i = 0; fb->format->num_planes && i < fb->format->num_planes; i++) {
		...
		case DRM_FORMAT_MOD_APPLE_INTERCHANGE:
			...
			surf->planes[i].tile_w = l.tile_w;
			surf->planes[i].tile_h = l.tile_h;
			surf->planes[i].stride = l.width_tiles * l.tile_bytes;
			...
			surf->compression_info[i] = (struct dcp_compression_info) {
				.tile_w = l.tile_w,
				.tile_h = l.tile_h,
				.data_offset = surf->planes[i].offset,
				.metadata_offset = surf->planes[i].offset + l.meta_offset,
				.meta_bytes = l.meta_tile_bytes,
				.tiles_w = l.width_tiles,
				.tiles_h = l.height_tiles,
				.tile_bytes = l.tile_bytes,
				.row_stride = l.width_tiles * l.tile_bytes,
...
u64 apple_format_modifiers[] = {
	DRM_FORMAT_MOD_APPLE_INTERCHANGE,
